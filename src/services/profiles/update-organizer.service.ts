...
export const updateOrganizerService = async (
  ...
  if (inputData.paymentTarget) {
    inputData.paymentTarget = inputData.paymentTarget;
    if (
      isNaN(Number(inputData.paymentTarget)) ||
      Number(inputData.paymentTarget) < 0
    ) {
  ...
  if (picture) {
    const { secure_url } = await cloudinaryUpload(picture, "organizer/profile");
