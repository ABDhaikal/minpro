...
export const uploadPaymentProofService = async (
  ...
  try {
    ...
    try {
        const uploadResult = await cloudinaryUpload(imageTransactionFile, "payment-proofs");
    ...
    const paymentProof = await prisma.payment.create({
      data: {
        transactionId: transactionId,
        imageTransaction: secureUrl,
        description: "Payment proof",
        organizerDeadline: transaction.paymentDeadline,
        createdAt: new Date(),
      },
    });
